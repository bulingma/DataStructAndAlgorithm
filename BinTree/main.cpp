...
template <typename T> void testBinTree(int h) { //测试二叉树
	...
	bt.travPre((Double<T>&) Double<T>());
	bt.travPre((Increase<T>&) Increase<T>());
	print(bt);

	bt.travIn((Double<T>&) Double<T>());
	bt.travIn((Increase<T>&) Increase<T>());
	print(bt);

	bt.travPost((Double<T>&) Double<T>());
	bt.travPost((Increase<T>&) Increase<T>());
	print(bt);

	bt.travLevel((Double<T>&) Double<T>());
	bt.travLevel((Increase<T>&) Increase<T>());
	print(bt);
...
int main(void) { //测试二叉树
	...
	testBinTree<int>(atoi(strNumber.c_str())); //元素类型可以在这里任意选择
	return 0;
}